...
SELECT
    ...
    JSON_VALUE([DestinationConnectionSettings], '$.lakehouseConnectionID')      AS [LakehouseConnectionID],
    JSON_VALUE([DestinationConnectionSettings], '$.lakehouseID')                AS [LakehouseID],            /* @item().LakehouseID */
    JSON_VALUE([DestinationConnectionSettings], '$.sqlConnectionString')        AS [SQLConnectionString],
    ...
    CASE JSON_VALUE([DataLoadingBehaviorSettings], '$.watermarkColumnType')
        ...
        END + ')) AS [CurrentMaxWaterMarkColumnValue] FROM [' + JSON_VALUE([DestinationObjectSettings], '$.schema') + '].[' + JSON_VALUE([DestinationObjectSettings], '$.table') + ']'
    AS [WatermarkQuery], /* @item().WatermarkQuery  */
